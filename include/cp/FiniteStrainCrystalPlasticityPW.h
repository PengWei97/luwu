...
/**
 * FiniteStrainCrystalPlasticityPW uses the multiplicative decomposition of deformation gradient
 * and solves the PK2 stress residual equation at the intermediate configuration to evolve the
 * material state.
 * The internal variables are updated using an interative predictor-corrector algorithm.
 * Backward Euler integration rule is used for the rate equations.
 */
class FiniteStrainCrystalPlasticityPW : public ComputeStressBase
{
public:
  static InputParameters validParams();

  FiniteStrainCrystalPlasticityPW(const InputParameters & parameters);
