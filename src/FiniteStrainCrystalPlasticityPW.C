...
#include "FiniteStrainCrystalPlasticityPW.h"
...
registerMooseObject("TensorMechanicsApp", FiniteStrainCrystalPlasticityPW);

InputParameters
FiniteStrainCrystalPlasticityPW::validParams()
{
...
FiniteStrainCrystalPlasticityPW::FiniteStrainCrystalPlasticityPW(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _crysrot(getMaterialProperty<RankTwoTensor>("crysrot")), // 通过欧拉角获取的旋转张量
...
void
FiniteStrainCrystalPlasticityPW::initQpStatefulProperties()
{
  _stress[_qp].zero();
...
void
FiniteStrainCrystalPlasticityPW::initSlipSysProps()
{
...
void
FiniteStrainCrystalPlasticityPW::assignSlipSysRes()
{
...
void
FiniteStrainCrystalPlasticityPW::readFileInitSlipSysRes()
{
  _gss[_qp].resize(_nss);
  ...
  for (unsigned int i = 0; i < _nss; ++i)
    if (!(file >> _gss[_qp][i]))
      mooseError("Error FiniteStrainCrystalPlasticityPW: Premature end of slip_sys_res_prop file");
...
void
FiniteStrainCrystalPlasticityPW::getInitSlipSysRes()
{
  if (_gprops.size() <= 0)
    mooseError("FiniteStrainCrystalPlasticityPW: Error in reading slip system resistance properties: "
  ...
  for (unsigned int i = 0; i < _gprops.size() / num_data_grp; ++i)
  {
    ...
    if (vs <= 0 || ve <= 0)
      mooseError("FiniteStrainCrystalPlasticityPW: Indices in gss property read must be positive "
                 "integers: is = ",
                 vs,
                 " ie = ",
                 ve);
    ...
    if (vs != floor(vs) || ve != floor(ve))
      mooseError("FiniteStrainCrystalPlasticityPW: Error in reading slip system resistances: Values "
    ...
    if (is > ie)
      mooseError("FiniteStrainCrystalPlasticityPW: Start index is = ",
                 is,
  ...
  for (unsigned int i = 0; i < _nss; ++i)
    if (_gss[_qp][i] <= 0.0)
      mooseError("FiniteStrainCrystalPlasticityPW: Value of resistance for slip system ",
                 i + 1,
                 " non positive");
}
...
void
FiniteStrainCrystalPlasticityPW::readFileFlowRateParams()
{
  _a0.resize(_nss);
  _xm.resize(_nss);
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    for (unsigned int j = 0; j < _num_slip_sys_flowrate_props; ++j)
      if (!(file >> vec[j]))
        mooseError(
            "Error FiniteStrainCrystalPlasticityPW: Premature end of slip_sys_flow_rate_param file");
...
void
FiniteStrainCrystalPlasticityPW::getFlowRateParams()
{
  if (_flowprops.size() <= 0)
    mooseError("FiniteStrainCrystalPlasticityPW: Error in reading flow rate  properties: Specify "
  ...
  for (unsigned int i = 0; i < _flowprops.size() / num_data_grp; ++i)
  {
    ...
    if (vs <= 0 || ve <= 0)
      mooseError("FiniteStrainCrystalPlasticityPW: Indices in flow rate parameter read must be "
    ...
    if (vs != floor(vs) || ve != floor(ve))
      mooseError("FiniteStrainCrystalPlasticityPW: Error in reading flow props: Values specifying "
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    ...
    {
      mooseWarning(
          "FiniteStrainCrystalPlasticityPW: Non-positive flow rate parameters ", _a0(i), ",", _xm(i));
      break;
    }
  }
}
...
void
FiniteStrainCrystalPlasticityPW::readFileHardnessParams()
{
}

// Read hardness parameters from .i file
void
FiniteStrainCrystalPlasticityPW::getHardnessParams()
{
  if (_hprops.size() <= 0)
    mooseError("FiniteStrainCrystalPlasticityPW: Error in reading hardness properties: Specify input "
...
void
FiniteStrainCrystalPlasticityPW::getSlipSystems()
{
...
void
FiniteStrainCrystalPlasticityPW::initAdditionalProps()
{
}
 ...
 */
void
FiniteStrainCrystalPlasticityPW::computeQpStress()
{
  ...
  while (_err_tol && _max_substep_iter > 1)
  {
    ...
    if (substep_iter > _max_substep_iter)
      mooseWarning("FiniteStrainCrystalPlasticityPW: Failure with substepping");
#endif
...
void
FiniteStrainCrystalPlasticityPW::preSolveQp()
{
  // Initialize variable
  if (_first_substep)
  {
...
void
FiniteStrainCrystalPlasticityPW::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
FiniteStrainCrystalPlasticityPW::postSolveQp()
{
  if (_err_tol)
  {
    _err_tol = false;
    if (_gen_rndm_stress_flag)
    {
      ...
      mooseError("FiniteStrainCrystalPlasticityPW: Constitutive failure");
  }
  else
  {
...
void
FiniteStrainCrystalPlasticityPW::preSolveStatevar()
{
...
void
FiniteStrainCrystalPlasticityPW::solveStatevar()
{
  Real gmax, gdiff;
  unsigned int iterg;
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainCrystalPlasticityPW: Hardness Integration error gmax", gmax, "\n");
#endif
    _err_tol = true;
  }
}
...
void
FiniteStrainCrystalPlasticityPW::postSolveStatevar()
{
  if (_max_substep_iter == 1) // No substepping
  {
    _gss[_qp] = _gss_tmp;
...
void
FiniteStrainCrystalPlasticityPW::preSolveStress()
{
  if (_max_substep_iter == 1) // No substepping
  {
...
void
FiniteStrainCrystalPlasticityPW::solveStress()
{
  unsigned int iter = 0;
...
#ifdef DEBUG
    mooseWarning(
        "FiniteStrainCrystalPlasticityPW: Slip increment exceeds tolerance - Element number ",
        _current_elem->id(),
        " Gauss point = ",
        _qp);
#endif
         ...
         iter < _maxiter) // Check for stress residual tolerance
  {
    ...
    {
#ifdef DEBUG
      mooseWarning("FiniteStrainCrystalPlasticityPW: Failed with line search");
#endif
      _err_tol = true;
      return;
    }
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainCrystalPlasticityPW: Stress Integration error rmax = ", rnorm);
#endif
    _err_tol = true;
  }
}
...
void
FiniteStrainCrystalPlasticityPW::postSolveStress()
{
  if (_max_substep_iter == 1) // No substepping
  {
...
void
FiniteStrainCrystalPlasticityPW::update_slip_system_resistance()
{
  updateGss();
}
 ...
 */
void
FiniteStrainCrystalPlasticityPW::updateGss()
{
...
void
FiniteStrainCrystalPlasticityPW::calc_resid_jacob(RankTwoTensor & resid, RankFourTensor & jac)
{
  calcResidual(resid);
  if (_err_tol)
    return;
...
void
FiniteStrainCrystalPlasticityPW::calcResidual(RankTwoTensor & resid)
{
...
void
FiniteStrainCrystalPlasticityPW::calcJacobian(RankFourTensor & jac)
{
...
void
FiniteStrainCrystalPlasticityPW::getSlipIncrements()
{
  for (unsigned int i = 0; i < _nss; ++i)
  {
...
RankTwoTensor
FiniteStrainCrystalPlasticityPW::get_current_rotation(const RankTwoTensor & a)
{
  return getMatRot(a);
}
...
RankTwoTensor
FiniteStrainCrystalPlasticityPW::getMatRot(const RankTwoTensor & a)
{
  RankTwoTensor rot;
  ...
  if (info != 0)
    mooseError("FiniteStrainCrystalPlasticityPW: DSYEV function call in getMatRot function failed");
...
void
FiniteStrainCrystalPlasticityPW::computeQpElasticityTensor()
{
}

RankFourTensor
FiniteStrainCrystalPlasticityPW::calcTangentModuli()
{
...
void
FiniteStrainCrystalPlasticityPW::calc_schmid_tensor()
{
...
RankFourTensor
FiniteStrainCrystalPlasticityPW::elastoPlasticTangentModuli()
{
  RankFourTensor tan_mod;
...
RankFourTensor
FiniteStrainCrystalPlasticityPW::elasticTangentModuli()
{
  return _elasticity_tensor[_qp]; // update jacobian_mult
}

bool
FiniteStrainCrystalPlasticityPW::line_search_update(const Real rnorm_prev, const RankTwoTensor dpk2)
{
...
void
FiniteStrainCrystalPlasticityPW::internalVariableUpdateNRiteration()
{
