...
template <>
void
dataStore(std::ostream & stream, FeatureFloodCountGG::FeatureData & feature, void * context)
{
  /**
...
template <>
void
dataLoad(std::istream & stream, FeatureFloodCountGG::FeatureData & feature, void * context)
{
  /**
...
registerMooseObject("PhaseFieldApp", FeatureFloodCountGG);

InputParameters
FeatureFloodCountGG::validParams()
{
  ...
  /**
   * The FeatureFloodCountGG and derived objects should not to operate on the displaced mesh. These
  ...
  // The FeatureFloodCountGG object does not require that any state (restartable information) is
...
FeatureFloodCountGG::FeatureFloodCountGG(const InputParameters & parameters)
...
void
FeatureFloodCountGG::initialSetup()
{
...
void
FeatureFloodCountGG::initialize()
{
...
void
FeatureFloodCountGG::clearDataStructures()
{
}

void
FeatureFloodCountGG::meshChanged()
{
...
FeatureFloodCountGG::execute()
{
  ...
  {
    mooseInfo("Using EXPERIMENTAL boundary restricted FeatureFloodCountGG object!\n");
...
processor_id_type
FeatureFloodCountGG::numberOfDistributedMergeHelpers() const
{
  return _app.n_processors() >= _maps_size ? _maps_size : 1;
}

void
FeatureFloodCountGG::communicateAndMerge()
{
  ...
  if (n_merging_procs > 1)
  {
    ...
    {
      /**
       * The FeatureFloodCountGG and derived objects rely on having the original data structures
...
void
FeatureFloodCountGG::sortAndLabel()
{
...
void
FeatureFloodCountGG::buildLocalToGlobalIndices(std::vector<std::size_t> & local_to_global_all,
                                             std::vector<int> & counts) const
{
  ...
  local_to_global_all.resize(globalsize, FeatureFloodCountGG::invalid_size_t);
  for (const auto & feature : _feature_sets)
  {
...
void
FeatureFloodCountGG::buildFeatureIdToLocalIndices(unsigned int max_id)
{
...
FeatureFloodCountGG::getVarToFeatureVector(dof_id_type elem_id) const
{
...
void
FeatureFloodCountGG::scatterAndUpdateRanks()
{
  ...
  if (!_is_primary)
  {
    ...
    for (auto & list_ref : _partial_feature_sets)
    {
      for (auto & feature : list_ref)
      {
        ...
        auto global_index = FeatureFloodCountGG::invalid_size_t;
        ...
        if (global_index != FeatureFloodCountGG::invalid_size_t)
        {
  ...
  else
  {
    for (auto global_index : local_to_global_all)
      if (global_index != FeatureFloodCountGG::invalid_size_t && global_index > largest_global_index)
        largest_global_index = global_index;
  }
...
Real
FeatureFloodCountGG::getValue()
{
  return static_cast<Real>(_feature_count);
}

std::size_t
FeatureFloodCountGG::getNumberActiveFeatures() const
{
  // Note: This value is parallel consistent, see FeatureFloodCountGG::communicateAndMerge()
  return _feature_count;
}

std::size_t
FeatureFloodCountGG::getTotalFeatureCount() const
{
  /**
   * Since the FeatureFloodCountGG object doesn't maintain any information about
...
unsigned int
FeatureFloodCountGG::getFeatureVar(unsigned int feature_id) const
{
...
bool
FeatureFloodCountGG::doesFeatureIntersectBoundary(unsigned int feature_id) const
{
  // TODO: This information is not parallel consistent when using FeatureFloodCountGGer
...
bool
FeatureFloodCountGG::doesFeatureIntersectSpecifiedBoundary(unsigned int feature_id) const
{
  // TODO: This information is not parallel consistent when using FeatureFloodCountGGer
...
bool
FeatureFloodCountGG::isFeaturePercolated(unsigned int feature_id) const
{
  // TODO: This information is not parallel consistent when using FeatureFloodCountGGer
...
Point
FeatureFloodCountGG::featureCentroid(unsigned int feature_id) const
{
  if (feature_id >= _feature_id_to_local_index.size())
    return invalid_id;
...
Real
FeatureFloodCountGG::getEntityValue(dof_id_type entity_id,
...
void
FeatureFloodCountGG::prepareDataForTransfer()
{
  ...
  for (auto & list_ref : _partial_feature_sets)
  {
    for (auto & feature : list_ref)
    {
       ...
       */
      FeatureFloodCountGG::sort(feature._ghosted_ids);
      FeatureFloodCountGG::sort(feature._local_ids);
      FeatureFloodCountGG::sort(feature._halo_ids);
      FeatureFloodCountGG::sort(feature._disjoint_halo_ids);
      FeatureFloodCountGG::sort(feature._periodic_nodes);
...
void
FeatureFloodCountGG::serialize(std::string & serialized_buffer, unsigned int var_num)
{
...
void
FeatureFloodCountGG::deserialize(std::vector<std::string> & serialized_buffers, unsigned int var_num)
{
...
void
FeatureFloodCountGG::consolidateMergedFeatures(std::vector<std::list<FeatureData>> * saved_data)
{
  ...
  /**
   * IMPORTANT: FeatureFloodCountGG::_feature_count is set on rank 0 at this point but
   * we can't broadcast it here because this routine is not collective.
   */
}

bool
FeatureFloodCountGG::areFeaturesMergeable(const FeatureData & f1, const FeatureData & f2) const
{
  return f1.mergeable(f2);
}

void
FeatureFloodCountGG::updateFieldInfo()
{
  for (const auto i : index_range(_feature_sets))
  {
...
bool
FeatureFloodCountGG::flood(const DofObject * dof_object, std::size_t current_index)
...
Real FeatureFloodCountGG::getThreshold(std::size_t /*current_index*/) const
{
  return _step_threshold;
}

Real FeatureFloodCountGG::getConnectingThreshold(std::size_t /*current_index*/) const
{
  return _step_connecting_threshold;
}

bool
FeatureFloodCountGG::compareValueWithThreshold(Real entity_value, Real threshold) const
{
...
bool
FeatureFloodCountGG::isNewFeatureOrConnectedRegion(const DofObject * dof_object,
...
void
FeatureFloodCountGG::expandPointHalos()
{
...
void
FeatureFloodCountGG::expandEdgeHalos(unsigned int num_layers_to_expand)
{
...
void
FeatureFloodCountGG::visitElementalNeighbors(const Elem * elem,
                                           FeatureData * feature,
...
void
FeatureFloodCountGG::visitNodalNeighbors(const Node * node,
                                       FeatureData * feature,
                                       bool expand_halos_only)
{
...
template <typename T>
void
FeatureFloodCountGG::visitNeighborsHelper(const T * curr_entity,
...
void
FeatureFloodCountGG::updateBoundaryIntersections(FeatureData & feature) const
{
  if (_is_elemental)
  {
    for (auto entity : feature._local_ids)
...
void
FeatureFloodCountGG::appendPeriodicNeighborNodes(FeatureData & feature) const
{
  if (_is_elemental)
  {
    for (auto entity : feature._local_ids)
...
template <typename T>
bool
FeatureFloodCountGG::isBoundaryEntity(const T * entity) const
{
...
void
FeatureFloodCountGG::FeatureData::updateBBoxExtremes(MeshBase & mesh)
{
  ...
  FeatureFloodCountGG::reserve(all_primary_region_ids, _local_ids.size() + _halo_ids.size());
  ...
  FeatureFloodCountGG::reserve(set_union, _halo_ids.size() + _disjoint_halo_ids.size());
...
void
FeatureFloodCountGG::FeatureData::updateBBoxExtremes(BoundingBox & bbox, const BoundingBox & rhs_bbox)
{
...
bool
FeatureFloodCountGG::FeatureData::boundingBoxesIntersect(const FeatureData & rhs) const
{
...
bool
FeatureFloodCountGG::FeatureData::halosIntersect(const FeatureData & rhs) const
{
...
bool
FeatureFloodCountGG::FeatureData::periodicBoundariesIntersect(const FeatureData & rhs) const
{
...
bool
FeatureFloodCountGG::FeatureData::ghostedIntersect(const FeatureData & rhs) const
{
  return setsIntersect(
...
bool
FeatureFloodCountGG::FeatureData::mergeable(const FeatureData & rhs) const
{
...
bool
FeatureFloodCountGG::FeatureData::canConsolidate(const FeatureData & rhs) const
{
...
void
FeatureFloodCountGG::FeatureData::merge(FeatureData && rhs)
{
  ...
  FeatureFloodCountGG::reserve(set_union, _local_ids.size() + rhs._local_ids.size());
  std::set_union(_local_ids.begin(),
                 _local_ids.end(),
  ...
  set_union.clear();
  FeatureFloodCountGG::reserve(set_union, _periodic_nodes.size() + rhs._periodic_nodes.size());
  ...
  set_union.clear();
  FeatureFloodCountGG::reserve(set_union, _ghosted_ids.size() + rhs._ghosted_ids.size());
  ...
  set_union.clear();
  FeatureFloodCountGG::reserve(set_union, _disjoint_halo_ids.size() + rhs._disjoint_halo_ids.size());
  ...
  set_union.clear();
  FeatureFloodCountGG::reserve(set_union, _halo_ids.size() + rhs._halo_ids.size());
  std::set_union(_halo_ids.begin(),
                 _halo_ids.end(),
                 rhs._halo_ids.begin(),
                 rhs._halo_ids.end(),
...
void
FeatureFloodCountGG::FeatureData::consolidate(FeatureData && rhs)
{
  mooseAssert(_var_index == rhs._var_index, "Mismatched variable index in merge");
  mooseAssert(_id == rhs._id, "Mismatched auxiliary id in merge");

  FeatureData::container_type set_union;
  FeatureFloodCountGG::reserve(_local_ids, _local_ids.size() + rhs._local_ids.size());
  std::set_union(_local_ids.begin(),
                 _local_ids.end(),
                 rhs._local_ids.begin(),
                 rhs._local_ids.end(),
...
void
FeatureFloodCountGG::FeatureData::clear()
{
  _local_ids.clear();
...
void
FeatureFloodCountGG::FeatureData::mergeBBoxes(std::vector<BoundingBox> & bboxes,
...
std::ostream &
operator<<(std::ostream & out, const FeatureFloodCountGG::FeatureData & feature)
{
  static const bool debug = true;

  out << "Grain ID: ";
  if (feature._id != FeatureFloodCountGG::invalid_id)
    out << feature._id;
  else
    out << "invalid";
  ...
  out << "\nStatus: ";
  if (feature._status == FeatureFloodCountGG::Status::CLEAR)
    out << "CLEAR";
  if (static_cast<bool>(feature._status & FeatureFloodCountGG::Status::MARKED))
    out << " MARKED";
  if (static_cast<bool>(feature._status & FeatureFloodCountGG::Status::DIRTY))
    out << " DIRTY";
  if (static_cast<bool>(feature._status & FeatureFloodCountGG::Status::INACTIVE))
    out << " INACTIVE";
...
// Constants
const std::size_t FeatureFloodCountGG::invalid_size_t = std::numeric_limits<std::size_t>::max();
const unsigned int FeatureFloodCountGG::invalid_id = std::numeric_limits<unsigned int>::max();
const processor_id_type FeatureFloodCountGG::invalid_proc_id =
