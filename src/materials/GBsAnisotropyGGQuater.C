...
#include "GBsAnisotropyGGQuater.h"
#include "RotationTensor.h"
#include "MooseMesh.h"
#include <cmath>

registerMooseObject("luwuApp", GBsAnisotropyGGQuater);

InputParameters
GBsAnisotropyGGQuater::validParams()
{
  ...
  params.addRequiredParam<bool>("tbEnergy_anisotropy",
                                "The GB energy anisotropy would be considered if true");                                                    
...
GBsAnisotropyGGQuater::GBsAnisotropyGGQuater(const InputParameters & parameters)
  : Material(parameters),
    ...
    _tbEnergy_anisotropy(getParam<bool>("tbEnergy_anisotropy")),
    _T(coupledValue("T")),
    ...
    _vaild_OPNum(declareProperty<Real>("vaild_OPNum")),
    _twin_type(declareProperty<Real>("twin_type")),
    _kb(8.617343e-5),      // Boltzmann constant in eV/K
    _JtoeV(6.24150974e18), // Joule to eV conversion
    _degree(0.0174532925199433), // ./degree ~ radians to degrees
    _mu_qp(0.0),
...
void
GBsAnisotropyGGQuater::computeQpProperties()
{
...
void
GBsAnisotropyGGQuater::computerGBParameter()
{
  Real _sigma_base = _matrix_sigma;
  Real _mob_base = _matrix_mob;
  Real _Q_base = 0.23;
  _vaild_OPNum[_qp] = 0.0;
  _twin_type[_qp] = 0.0;
  ...
  for (unsigned int i = 0; i < _op_num; ++i)
  {
    ...
    _mob[i] = row_mob; // unit: m^4/(J*s) GB mobilityl
    _Q[i] = row_Q; // unit: eV
  }
  ...
  _vaild_OPNum[_qp] = grainIDIndex.size();
  _misori[_qp] = 0.0;
  if (grainIDIndex.size() == 2)
  {
    ...
    _s_misoriTwin = CalculateMisorientationAngle::calculateMisorientaion(angles_i, angles_j, _s_misoriTwin, _degree);
    _misori[_qp] = _s_misoriTwin.misor; // get misorientation angle
  }
      ...
      for (unsigned int j = i+1; j < grainIDIndex.size(); ++j)
        {
          ...
          _s_misoriTwin = CalculateMisorientationAngle::calculateMisorientaion(angles_i, angles_j, _s_misoriTwin, _degree);
          delta_euler = _s_misoriTwin.misor;

          // 情况1：如果判定为非孪晶界，或者没有考虑孪晶，就执行GB anisotropy
          if (!_s_misoriTwin.isTwinning || !_tbEnergy_anisotropy)
            _sigma[orderParameterIndex[i]][orderParameterIndex[j]] = _piecewise_func_sigma.sample(delta_euler);
          else if (_s_misoriTwin.isTwinning && _s_misoriTwin.twinType == "twin_type0")
          {
            _sigma[orderParameterIndex[i]][orderParameterIndex[j]] = _TT1_sigma;
            _twin_type[_qp] = 1.0;
          }
          else if (_s_misoriTwin.isTwinning && _s_misoriTwin.twinType == "twin_type1")
          {
            _sigma[orderParameterIndex[i]][orderParameterIndex[j]] = _CT1_sigma;
            _twin_type[_qp] = 2.0;
          }
...
void
GBsAnisotropyGGQuater::computerPhaseFieldParameter()
{
  Real sigma_init;
  Real g2 = 0.0;
  Real f_interf = 0.0;
...
// /es01/paratera/sce0895/luwu/include/vectorpostprocessors/FeatureDataVectorPostprocessor.h