...
#include "GNGsGrainGrowth.h"
#include "MooseMesh.h"

registerMooseObject("luwuApp", GNGsGrainGrowth);

#include <fstream>

InputParameters
GNGsGrainGrowth::validParams()
{
  ...
  //                                               "The name of the VectorPostprocessor in the panda to transfer values from FeatureVolumeVectorPostprocessor.");
  ...
  params.addParam<Real>("GBsigma_HAB",0.52, "the gb energy of a high angle grain boudary");
...
GNGsGrainGrowth::GNGsGrainGrowth(const InputParameters & parameters)
  : Material(parameters),
    _grain_tracker(getUserObject<GrainTracker>("grain_tracker")),
    // _vector_postprocessor_name(getParam<VectorPostprocessorName>("vector_postprocessor")),
...
GNGsGrainGrowth::computeQpProperties()
{
  ...
  for (unsigned int m = 0; m < _op_num - 1; ++m)
  {
    for (unsigned int n = m + 1; n < _op_num; ++n) // m<n
    {
      ...
      // sum_L += Val * _mob[m][n] * _mu_qp * _a_g2[n][m] / _sigma[m][n];
    }
  }
...
void
GNGsGrainGrowth::computeGBParamaterByGrainSize()
{
  // Get list of active order parameters from grain tracker, std::vector<unsigned int>
  const std::vector<unsigned int> & op_to_grains = _grain_tracker.getVarToFeatureVector(_current_elem->id()); // the vector of the order parameters, the value of op_to_grains[i] is the grain id 
  ...
  Real GBsigma_LOW = _GBsigma_HAB;
  Real GBmob_LOW = _GBmob_HAB;
  Real GBQ_LOW = _GBQ_HAB;

  // 对sigma_ij, mob_ij, Q_ij进行初始化操作
...
void
GNGsGrainGrowth::computeGBParamater()
{
  Real sigma_init; // the initial value of GB energy
  Real g2 = 0.0; // g^2(gamma*)
  Real f_interf = 0.0; // f_{0,initerf}
  Real a_0 = 0.75; // a
  Real a_star = 0.0; // a*
  Real kappa_star = 0.0; // kappa*
  Real gamma_star = 0.0; // gamma*
  Real y = 0.0; // 1/gamma
  Real yyy = 0.0;
  ...
  sigma_init = (sigma_big + sigma_small) / 2.0; // GB energy
  _mu_qp = 6.0 * sigma_init / _wGB; // eV/Xm^2/Xm-- eV/Xm^3 local free energy pre-factor
